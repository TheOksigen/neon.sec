...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "Bu istifadəçi N Σ O N Team-ın bir parçasıdır\nOna qarşı bunu edə bilmərəm."
        )
        return
...
def ungban(update: Update, context: CallbackContext):
    ...
    message.reply_text(
        f"Mən {user_chat.first_name} istifadəçisinə qlobal olaraq 2-ci şans verirəm yaxsi qurtardn XD.")
...
*Qeyd:* @Neonsup qrupunda belə istifadəçiləri şikayət edə bilərsiniz
